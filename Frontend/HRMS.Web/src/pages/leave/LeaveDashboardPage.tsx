...
export function LeaveDashboardPage() {
  ...
  const displayName = user?.employeeIdentity?.employee?.displayName ?? user?.fullName ?? 'you'
  if (!linked) return <div className="leave-admin-page leave-dashboard-page"><DashboardHero displayName={displayName} period={null} /><div className="leave-dashboard-grid">{canViewHrDashboard && <HrDashboardCard query={hrDashboard} />}<Card className="leave-dashboard-identity-card"><DashboardEmpty icon="user" title="Leave access needs an employee link" message="Your account is not linked to an active employee. Contact HR before applying for or viewing Leave." /></Card></div></div>
  ...
  return <div className="leave-admin-page leave-dashboard-page">
    <DashboardHero displayName={displayName} period={hrDashboard.data ? { from: hrDashboard.data.from, to: hrDashboard.data.to, name: hrDashboard.data.currentLeavePeriodName } : null} />
    <QuickActions canApprove={canApprove} canConfigure={can(Permissions.leave.policyView)} />
...
function DashboardHero({ displayName, period }: { displayName: string; period: { from: string; to: string; name?: string | null } | null }) {
  return <header className="leave-dashboard-hero"><div className="leave-dashboard-hero-copy"><span className="leave-dashboard-eyebrow">Welcome back</span><h1>Leave Dashboard</h1><p>A quick overview for {displayName}</p><span className="leave-dashboard-hero-support">Manage your leave requests, balances and team updates.</span></div>{period && <div className="leave-dashboard-period"><span>Leave year</span><strong>{period.from} – {period.to}</strong>{period.name && <small>{period.name}</small>}</div>}</header>
}

function HrDashboardCard({ query }: { query: { data: HrLeaveDashboardSummary | null; error: { message: string } | null; isLoading: boolean } }) {
  const data = query.data
  return <Card className="leave-dashboard-admin-card" title={<span className="card-title-with-icon"><span className="card-icon is-purple"><Icon name="chart" /></span>HR / Admin overview</span>} subtitle={data ? `${data.from} → ${data.to}${data.currentLeavePeriodName ? ` · ${data.currentLeavePeriodName}` : ''}` : 'Authorized Leave operations'} actions={<Link className="row-action" to="/leave-management/team-calendar">Open calendar</Link>}>
    {query.isLoading ? <DashboardSkeleton label="Loading HR Leave dashboard" /> : query.error ? <WidgetError message={query.error.message} /> : !data ? <DashboardEmpty icon="chart" title="No HR dashboard data" /> : <>
      <div className="leave-dashboard-stat-grid"><SummaryStat icon="users" tone="teal" label="On Leave today" value={`${data.kpis.employeesOnLeaveToday} / ${data.kpis.activeEmployeeCount}`} hint="Active employees" href="/leave-management/team-calendar" /><SummaryStat icon="clock" tone="amber" label="Pending approvals" value={data.kpis.pendingApprovalRequests} hint="Authorized scope" href="/leave-management/approvals" /><SummaryStat icon="calendar" tone="blue" label="Upcoming Leave" value={data.kpis.upcomingApprovedRequests} hint="Next 30 days" href="/leave-management/team-calendar" /><SummaryStat icon="check" tone="green" label="Approved in scope" value={data.kpis.approvedRequests} hint="Approved requests" href="/leave-management/team-calendar" /></div>
      <div className="leave-dashboard-admin-lists"><DashboardList title="Leave type usage" icon="layers" rows={data.leaveTypeUsage.map(item => `${item.name}: ${formatQuantity(item.quantity)} day(s) · ${item.requestCount} request(s)`)} empty="No approved Leave usage in this period." /><DashboardList title="Request status" icon="clipboard" rows={data.statusBreakdown.map(item => `${statusLabels[item.status]}: ${item.requestCount} · ${formatQuantity(item.quantity)} day(s)`)} empty="No requests in this period." /><DashboardList title="Approval aging" icon="clock" rows={data.approvalAging.map(item => `${item.bucket}: ${item.requestCount}`)} empty="No pending approvals." /><DashboardList title="Upcoming absences" icon="calendar" rows={data.upcomingAbsences.slice(0, 5).map(item => `${item.employeeName || item.employeeCode} · ${item.leaveTypeName} · ${item.startDate} → ${item.endDate}`)} empty="No upcoming approved Leave." /></div>
    </>}
  </Card>
}

function DashboardList({ title, icon, rows, empty }: { title: string; icon: IconName; rows: string[]; empty: string }) {
  return <section className="leave-dashboard-admin-list"><h3><Icon name={icon} />{title}</h3>{rows.length ? <ul className="leave-dashboard-list">{rows.map(row => <li key={row}><span>{row}</span></li>)}</ul> : <p className="muted">{empty}</p>}</section>
}
...
function QuickActions({ canApprove, canConfigure }: { canApprove: boolean; canConfigure: boolean }) {
  ...
  const subtitles: Record<string, string> = { 'Apply Leave': 'Submit a new leave request', 'My Leave Requests': 'View and track your requests', 'Team Leave Calendar': 'View your team\'s leave', 'Manager Approvals': 'Review requests assigned to you', 'Leave Policy': 'Review leave policy rules' }
  return <nav className="leave-dashboard-actions" aria-label="Leave quick actions">{actions.map(action => <Link className={`leave-dashboard-action ${action.primary ? 'is-primary' : ''}`} to={action.to} key={action.to}><span className="leave-dashboard-action-icon"><Icon name={action.icon as IconName} /></span><span><strong>{action.label}</strong><small>{subtitles[action.label]}</small></span><span className="action-arrow" aria-hidden="true">→</span></Link>)}</nav>
}
...
function WidgetError({}: { message: string }) { return <div className="dashboard-widget-error"><span>Unable to load this section. Please try again.</span><button type="button" className="button button-link" onClick={() => window.location.reload()}>Retry</button></div> }
