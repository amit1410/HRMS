...
function BalanceCard({ query }: { query: { data: LeaveBalanceSummary[] | null; error: { message: string } | null; isLoading: boolean } }) {
  return <Card title="Leave balances" subtitle="Current finite balance projections">{query.isLoading ? <div className="state-block"><Spinner label="Loading Leave balances" /></div> : query.error ? <Notice tone="error">{query.error.message}</Notice> : query.data?.length ? <div className="leave-balance-list">{query.data.map(balance => <div className="leave-balance-item" key={balance.balanceId}><div><strong>{balance.leaveTypeName}</strong><span className="muted">{balance.leaveTypeCode} · {balance.leavePeriodName}</span></div><strong>{formatQuantity(balance.availableQuantity)} available</strong><dl><div><dt>Allocated</dt><dd>{formatQuantity(balance.grantedQuantity)}</dd></div><div><dt>Reserved</dt><dd>{formatQuantity(balance.reservedQuantity)}</dd></div><div><dt>Used</dt><dd>{formatQuantity(balance.consumedQuantity)}</dd></div></dl></div>)}</div> : <EmptyState title="No balance allocations" message="No finite Leave balance has been initialized for your current employee record." />}</Card>
}
