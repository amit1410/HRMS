...
export function LeavePoliciesPage() {
  ...
  function openCreate() { setRequestedEditId(null); setSearchParams({}, { replace: true }); setEditing(null); setEditorOpen(true); setForm({ ...emptyForm }); setError(null); setNotice(null) }
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setSaving(true); setError(null); setNotice(null); try { if (editing) { await updateLeavePolicy(editing.id, form); setRequestedEditId(null); setSearchParams({}, { replace: true }); setEditing(null); setEditorOpen(false); setNotice('Leave Policy updated.'); query.refetch() } else { const created = await createLeavePolicy(form); setRequestedEditId(null); setSearchParams({}, { replace: true }); setEditing(null); setEditorOpen(false); navigate(`/leave-management/policies/${created.id}`) } } catch (caught) { setError(caught instanceof ApiError ? caught : new ApiError('Unable to save Leave Policy.')) } finally { setSaving(false) } }
