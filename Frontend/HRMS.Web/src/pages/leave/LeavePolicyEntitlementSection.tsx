...
export function LeavePolicyEntitlementSection({ policyId, version, leaveTypes, canManage, onNotice, onChanged }: Props) {
  ...
  return <Card title="6. Entitlement" subtitle="Configure the quantity and credit source for each selected Leave Type. Runtime balances and credits are deferred.">
    {leaveTypes.length === 0 ? <p className="muted">Assign at least one Leave Type before configuring Entitlement.</p> : <>
      ...
      {loading ? <Spinner label="Loading Entitlement" /> : <>{!editable && <div className="lifecycle-notice"><Badge>{version.status}</Badge><span>{version.status === 'Published' ? 'Published Entitlement is immutable.' : version.status === 'Retired' ? 'Retired Entitlement is retained for history.' : 'You can view Entitlement but do not have edit permission.'}</span></div>}<form className="form-stack" aria-label={`${selectedType?.name ?? 'Leave Type'} Entitlement`} onSubmit={event => { event.preventDefault(); void save() }}><fieldset disabled={!editable} className="eligibility-fields"><legend>Entitlement configuration</legend><label className="field"><span>Entitlement Mode</span><select className="input" value={form.entitlementMode} onChange={event => updateMode(event.target.value as EntitlementMode)}><option value="Allocated">Allocated</option><option value="Unlimited">Unlimited</option><option value="NoBalanceRequired">No balance required</option></select></label><label className="field"><span>Entitlement Source</span><select className="input" value={form.entitlementSource} onChange={event => update('entitlementSource', event.target.value as EntitlementSource)}><option value="PolicyAccrual">Policy accrual</option><option value="ExternalGrant">External grant</option><option value="NoBalanceRequired">No balance required</option></select></label>{form.entitlementMode === 'Allocated' ? <label className="field"><span>Entitlement Quantity <em>(required)</em></span><input className="input" type="number" min="0.001" step="0.001" required value={form.entitlementQuantity} onChange={event => update('entitlementQuantity', event.target.value)} /></label> : <p className="field-help">{form.entitlementMode === 'Unlimited' ? 'No finite allocation quantity is stored.' : 'This Leave Type does not consume a normal balance.'}</p>}<label className="field"><span>Accrual Frequency</span><select className="input" value={form.accrualFrequency} onChange={event => update('accrualFrequency', event.target.value as AccrualFrequency)}><option value="None">None</option><option value="Daily">Daily</option><option value="Monthly">Monthly</option><option value="SemiAnnual">Semi-annual</option><option value="Annual">Annual</option><option value="Yearly">Yearly</option></select></label>{form.accrualFrequency !== 'None' ? <><label className="field"><span>Accrual Timing</span><select className="input" value={form.accrualTiming} onChange={event => update('accrualTiming', event.target.value as AccrualTiming)}><option value="StartOfPeriod">Start of period</option><option value="EndOfPeriod">End of period</option></select></label><label className="field"><span><input type="checkbox" checked={form.proratePartialPeriod} onChange={event => update('proratePartialPeriod', event.target.checked)} /> Prorate partial period</span></label><label className="field"><span>Maximum accumulation (optional)</span><input className="input" type="number" min="0.001" step="0.001" value={form.maximumAccumulation} onChange={event => update('maximumAccumulation', event.target.value)} /></label><label className="field"><span><input type="checkbox" checked={form.carryForwardEnabled} onChange={event => update('carryForwardEnabled', event.target.checked)} /> Carry forward</span></label>{form.carryForwardEnabled ? <><label className="field"><span>Maximum carry-forward (optional)</span><input className="input" type="number" min="0.001" step="0.001" value={form.maximumCarryForwardQuantity} onChange={event => update('maximumCarryForwardQuantity', event.target.value)} /></label><label className="field"><span>Carry-forward expiry days (optional)</span><input className="input" type="number" min="1" step="1" value={form.carryForwardExpiryDays} onChange={event => update('carryForwardExpiryDays', event.target.value)} /></label></> : null}</> : null}</fieldset>{editable ? <div className="form-actions"><button className="button button-primary" type="submit" disabled={saving}>{saving ? <Spinner size={14} label="Saving…" /> : 'Save Entitlement'}</button></div> : null}</form></>}
    </>}
  </Card>
}
