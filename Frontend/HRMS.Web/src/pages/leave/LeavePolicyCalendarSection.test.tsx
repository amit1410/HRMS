...
describe('LeavePolicyCalendarSection', () => { let stub: StubAdapter; beforeEach(() => { stub = installStubAdapter() }); afterEach(() => stub.restore())
  it('keeps unsupported sandwich choices unavailable and saves the safe calendar settings', async () => { stub.on('get', url, () => ({ data: ok(null) })); stub.on('put', url, () => ({ data: ok(null) })); renderAsUser(<LeavePolicyCalendarSection policyId="policy-1" version={version} leaveTypes={[type]} canManage onNotice={() => undefined} />, { user: makeUser({ permissions: [Permissions.leave.policyManage] }) }); const form = await screen.findByRole('form', { name: 'Casual Leave Calendar Rules' }); expect(within(form).getByRole('option', { name: /Holiday \(Coming soon\)/ })).toBeDisabled(); await userEvent.click(within(form).getByRole('button', { name: 'Save Calendar Rules' })); expect(stub.callsTo('put', url)[0]?.body).toMatchObject({ sandwichMode: 'Disabled', applyToPrefix: false, applyToSuffix: false, applyToBetween: false }) })
