...
export function LeaveTypesPage() {
  ...
  function openCreate() { setEditingId(null); setEditorOpen(true); setForm({ ...emptyForm }); setError(null); setNotice(null); setDirty(false) }
  function openEdit(item: LeaveType) { setEditingId(item.id); setEditorOpen(true); setForm({ code: item.code, name: item.name, description: item.description ?? '', defaultUnit: item.defaultUnit, isPaid: item.isPaid, isActive: item.isActive, concurrencyToken: item.concurrencyToken }); setError(null); setNotice(null); setDirty(false) }
  function closeEditor() { if (dirty && !window.confirm('Discard unsaved changes?')) return; setEditingId(null); setEditorOpen(false); setDirty(false) }
  ...
  async function submit(event: FormEvent) {
    ...
    try { if (isEditing) await updateLeaveType(editingId, form); else await createLeaveType(form); setEditingId(null); setEditorOpen(false); setDirty(false); setNotice(isEditing ? 'Leave Type updated.' : 'Leave Type created.'); query.refetch() }
