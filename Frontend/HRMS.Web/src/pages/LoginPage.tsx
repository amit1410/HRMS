...
export function LoginPage() {
  ...
  const [identifier, setIdentifier] = useState(rememberedIdentifier?.identifier ?? ''); const [password, setPassword] = useState(''); const [passwordVisible, setPasswordVisible] = useState(false)
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null)
    try {
      const rememberMe = (event.currentTarget.elements.namedItem('rememberMe') as HTMLInputElement | null)?.checked ?? false
      session.setRememberMe(rememberMe)
      await login({ identifier: trimmedIdentifier, password })
      saveRememberedIdentifier(trimmedIdentifier, rememberMe)
...
const REMEMBERED_IDENTIFIER_KEY = 'hrms.rememberedIdentifier.v1'
function readRememberedIdentifier(): { identifier: string } | null { try { const identifier = window.localStorage.getItem(REMEMBERED_IDENTIFIER_KEY); return identifier ? { identifier } : null } catch { return null } }
function saveRememberedIdentifier(identifier: string, rememberMe: boolean): void { try { if (rememberMe) window.localStorage.setItem(REMEMBERED_IDENTIFIER_KEY, identifier); else window.localStorage.removeItem(REMEMBERED_IDENTIFIER_KEY) } catch { /* Browser autofill remains available when storage is unavailable. */ } }
