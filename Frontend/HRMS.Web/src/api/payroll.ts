...
export type LoanProductType = 'Loan' | 'SalaryAdvance'
export type LoanInterestMethod = 'None' | 'Flat' | 'ReducingBalance'
export type LoanRecoveryPolicy = 'RecoverFullOrFail' | 'PartialRecovery' | 'DeferInstallment'
export type LoanStatus = 'Draft' | 'Submitted' | 'Approved' | 'Rejected' | 'Disbursed' | 'Active' | 'Closed' | 'Cancelled'
export interface LoanProduct { id: string; code: string; name: string; productType: LoanProductType; isActive: boolean; currencyCode: string; minAmount?: number | null; maxAmount?: number | null; minTenureMonths?: number | null; maxTenureMonths?: number | null; interestMethod: LoanInterestMethod; interestRate?: number | null; recoveryPolicy: LoanRecoveryPolicy }
export interface LoanInstallment { id: string; installmentNumber: number; dueDate: string; openingPrincipal: number; principalAmount: number; interestAmount: number; installmentAmount: number; closingPrincipal: number; status: string; recoveredAmount: number }
export interface EmployeeLoan { id: string; employeeId: string; loanProductId: string; loanNumber: string; requestedAmount: number; approvedAmount?: number | null; disbursedAmount?: number | null; requestedTenureMonths: number; approvedTenureMonths?: number | null; status: LoanStatus; outstandingPrincipal: number; outstandingInterest: number; outstandingTotal: number; currencyCode: string; installments: LoanInstallment[] }
export interface LoanRegisterRow { id: string; loanNumber: string; employeeId: string; employeeCode: string; employeeName: string; productCode: string; productType: LoanProductType; approvedAmount: number; principalRecovered: number; interestRecovered: number; totalRecovered: number; outstandingPrincipal: number; outstandingInterest: number; outstandingTotal: number; nextDueDate?: string | null; status: LoanStatus }
export interface LoanProductRequest { code: string; name: string; productType: LoanProductType; isActive: boolean; currencyCode: string; minAmount?: number | null; maxAmount?: number | null; minTenureMonths?: number | null; maxTenureMonths?: number | null; interestMethod: LoanInterestMethod; interestRate?: number | null; allowPartialPrepayment: boolean; allowEarlyClosure: boolean; maxConcurrentLoans?: number | null; recoveryPolicy: LoanRecoveryPolicy }
export interface LoanProductVersionRequest { effectiveFrom: string; effectiveTo?: string | null; minAmount: number; maxAmount: number; minTenureMonths: number; maxTenureMonths: number; interestMethod: LoanInterestMethod; interestRate: number; allowPartialPrepayment: boolean; allowEarlyClosure: boolean; recoveryPolicy: LoanRecoveryPolicy }
export interface LoanRequest { employeeId: string; loanProductId: string; requestedAmount: number; requestedTenureMonths: number; currencyCode: string }
export interface LoanRepaymentRequest { amount: number; paymentDate?: string; reference?: string }
export function listLoanProducts(signal?: AbortSignal): Promise<LoanProduct[]> { return request<LoanProduct[]>(() => api.get<ApiResponse<LoanProduct[]>>('/api/payroll/loan-products', { signal })) }
export function createLoanProduct(body: LoanProductRequest): Promise<LoanProduct> { return request<LoanProduct>(() => api.post<ApiResponse<LoanProduct>>('/api/payroll/loan-products', body)) }
export function updateLoanProduct(id: string, body: LoanProductRequest): Promise<LoanProduct> { return request<LoanProduct>(() => api.put<ApiResponse<LoanProduct>>(`/api/payroll/loan-products/${id}`, body)) }
export function createLoanProductVersion(id: string, body: LoanProductVersionRequest): Promise<LoanProduct> { return request<LoanProduct>(() => api.post<ApiResponse<LoanProduct>>(`/api/payroll/loan-products/${id}/versions`, body)) }
export function listPayrollLoans(params: QueryParams = {}, signal?: AbortSignal): Promise<EmployeeLoan[]> { return request<EmployeeLoan[]>(() => api.get<ApiResponse<EmployeeLoan[]>>('/api/payroll/loans', { params: cleanParams(params), signal })) }
export function submitPayrollLoan(id: string): Promise<EmployeeLoan> { return request<EmployeeLoan>(() => api.post<ApiResponse<EmployeeLoan>>(`/api/payroll/loans/${id}/submit`)) }
export function approvePayrollLoan(id: string, amount?: number, tenureMonths?: number): Promise<EmployeeLoan> { return request<EmployeeLoan>(() => api.post<ApiResponse<EmployeeLoan>>(`/api/payroll/loans/${id}/approve`, undefined, { params: cleanParams({ amount, tenureMonths }) })) }
export function rejectPayrollLoan(id: string, reason: string): Promise<EmployeeLoan> { return request<EmployeeLoan>(() => api.post<ApiResponse<EmployeeLoan>>(`/api/payroll/loans/${id}/reject`, undefined, { params: { reason } })) }
export function recordLoanDisbursement(id: string, amount?: number): Promise<EmployeeLoan> { return request<EmployeeLoan>(() => api.post<ApiResponse<EmployeeLoan>>(`/api/payroll/loans/${id}/record-disbursement`, undefined, { params: cleanParams({ amount }) })) }
export function recordLoanRepayment(id: string, body: LoanRepaymentRequest): Promise<EmployeeLoan> { return request<EmployeeLoan>(() => api.post<ApiResponse<EmployeeLoan>>(`/api/payroll/loans/${id}/repayments`, body)) }
export function prepayLoan(id: string, body: LoanRepaymentRequest): Promise<EmployeeLoan> { return request<EmployeeLoan>(() => api.post<ApiResponse<EmployeeLoan>>(`/api/payroll/loans/${id}/prepay`, body)) }
export function closeLoan(id: string, body: LoanRepaymentRequest): Promise<EmployeeLoan> { return request<EmployeeLoan>(() => api.post<ApiResponse<EmployeeLoan>>(`/api/payroll/loans/${id}/close`, body)) }
export function cancelPayrollLoan(id: string, reason?: string): Promise<EmployeeLoan> { return request<EmployeeLoan>(() => api.post<ApiResponse<EmployeeLoan>>(`/api/payroll/loans/${id}/cancel`, undefined, { params: cleanParams({ reason }) })) }
export function getPayrollLoanSchedule(id: string): Promise<LoanInstallment[]> { return request<LoanInstallment[]>(() => api.get<ApiResponse<LoanInstallment[]>>(`/api/payroll/loans/${id}/schedule`)) }
export function listMyLoanProducts(signal?: AbortSignal): Promise<LoanProduct[]> { return request<LoanProduct[]>(() => api.get<ApiResponse<LoanProduct[]>>('/api/me/loan-products', { signal })) }
export function listMyLoans(signal?: AbortSignal): Promise<EmployeeLoan[]> { return request<EmployeeLoan[]>(() => api.get<ApiResponse<EmployeeLoan[]>>('/api/me/loans', { signal })) }
export function createMyLoan(body: Omit<LoanRequest, 'employeeId'>): Promise<EmployeeLoan> { return request<EmployeeLoan>(() => api.post<ApiResponse<EmployeeLoan>>('/api/me/loans', body)) }
export function submitMyLoan(id: string): Promise<EmployeeLoan> { return request<EmployeeLoan>(() => api.post<ApiResponse<EmployeeLoan>>(`/api/me/loans/${id}/submit`)) }
export function listLoanRegister(params: QueryParams = {}, signal?: AbortSignal): Promise<PagedResult<LoanRegisterRow>> { return request<PagedResult<LoanRegisterRow>>(() => api.get<ApiResponse<PagedResult<LoanRegisterRow>>>('/api/payroll/loans/register', { params: cleanParams(params), signal })) }
...
export type PayrollRetroStatus = 'Detected' | 'Evaluated' | 'Approved' | 'Applied' | 'Cancelled' | 'NoImpact'
export interface PayrollRetroCase { id: string; employeeId: string; triggerType: string; effectiveFrom: string; effectiveTo?: string | null; status: PayrollRetroStatus; results: unknown[] }
export interface FinalSettlement { id: string; employeeId: string; separationDate: string; lastWorkingDate: string; settlementDate: string; status: string; grossPayable: number; totalDeductions: number; netSettlement: number; currencyCode: string; lines: Array<{ id: string; lineType: string; componentCode: string; description: string; amount: number; isEarning: boolean; isDeduction: boolean }> }
export function createPayrollRetroCase(body: { employeeId: string; triggerType: string; effectiveFrom: string; effectiveTo?: string | null; notes?: string | null }): Promise<PayrollRetroCase> { return request<PayrollRetroCase>(() => api.post<ApiResponse<PayrollRetroCase>>('/api/payroll/retro/cases', body)) }
export function evaluatePayrollRetroCase(id: string): Promise<PayrollRetroCase> { return request<PayrollRetroCase>(() => api.post<ApiResponse<PayrollRetroCase>>(`/api/payroll/retro/cases/${id}/evaluate`)) }
export function createFinalSettlement(body: { employeeId: string; separationDate: string; lastWorkingDate: string; settlementDate: string; currencyCode: string }): Promise<FinalSettlement> { return request<FinalSettlement>(() => api.post<ApiResponse<FinalSettlement>>('/api/payroll/final-settlements', body)) }
export function addFinalSettlementLine(id: string, body: { lineType: string; componentCode: string; description: string; amount: number; isEarning: boolean; isDeduction: boolean }): Promise<FinalSettlement> { return request<FinalSettlement>(() => api.post<ApiResponse<FinalSettlement>>(`/api/payroll/final-settlements/${id}/lines`, body)) }
export function calculateFinalSettlement(id: string): Promise<FinalSettlement> { return request<FinalSettlement>(() => api.post<ApiResponse<FinalSettlement>>(`/api/payroll/final-settlements/${id}/calculate`)) }
export type PayrollCompliancePeriod = { id: string; jurisdictionCode: string; complianceType: string; periodStart: string; periodEnd: string; dueDate?: string | null; status: string }
export type PayrollStatutoryReturn = { id: string; compliancePeriodId: string; complianceType: string; jurisdictionCode: string; batchNumber: string; status: string; employeeCount: number; grossRelevantWages: number; employeeContribution: number; employerContribution: number; totalDeduction: number; totalPayable: number; employees: Array<{ id: string; employeeId: string; employeeCode: string; employeeName: string; employeeContribution: number; employerContribution: number; deductionAmount: number; payableAmount: number; validationStatus: string }> }
export function createPayrollCompliancePeriod(body: { jurisdictionCode: string; complianceType: string; periodStart: string; periodEnd: string; dueDate?: string | null }): Promise<PayrollCompliancePeriod> { return request<PayrollCompliancePeriod>(() => api.post<ApiResponse<PayrollCompliancePeriod>>('/api/payroll/statutory-compliance/periods', body)) }
export function generatePayrollStatutoryReturn(id: string): Promise<PayrollStatutoryReturn> { return request<PayrollStatutoryReturn>(() => api.post<ApiResponse<PayrollStatutoryReturn>>(`/api/payroll/statutory-compliance/periods/${id}/returns`)) }
export function getPayrollStatutoryReturn(id: string): Promise<PayrollStatutoryReturn> { return request<PayrollStatutoryReturn>(() => api.get<ApiResponse<PayrollStatutoryReturn>>(`/api/payroll/statutory-compliance/returns/${id}`)) }
export function validatePayrollStatutoryReturn(id: string): Promise<PayrollStatutoryReturn> { return request<PayrollStatutoryReturn>(() => api.post<ApiResponse<PayrollStatutoryReturn>>(`/api/payroll/statutory-compliance/returns/${id}/validate`)) }
