...
import { visibleNavGroups, visibleNavItems, type NavItem } from './navigation.ts'
...
export function Sidebar({ branding, open, onClose }: { branding: TenantBranding | null; open: boolean; onClose: () => void }) {
  ...
  const groups = visibleNavGroups(can, user)
  useEffect(() => {
    if (!open) return
  ...
  return (
    <nav className={`sidebar${open ? ' is-open' : ''}`} aria-label="Main">
      ...
      <ul className="nav-list"><li><NavEntry item={items.find(item => item.to === '/dashboard')!} onClose={onClose} /></li>{groups.map(group => <li key={group.id} className="nav-group"><details open><summary className="nav-group-summary">{group.label}</summary><ul className="nav-sublist">{group.items.map(item => <li key={item.to}><NavEntry item={item} onClose={onClose} /></li>)}</ul></details></li>)}</ul>
...
function NavEntry({ item, onClose }: { item: NavItem; onClose: () => void }) {
  return item.available ? <NavLink to={item.to} className={({ isActive }) => isActive ? 'nav-link is-active' : 'nav-link'} onClick={onClose}>{item.label === 'Change Password' ? <LockNavIcon /> : <NavItemIcon label={item.label} />}{item.label}</NavLink> : <span className="nav-link is-disabled" aria-disabled="true"><NavItemIcon label={item.label} />{item.label}<span className="nav-soon">soon</span></span>
}
