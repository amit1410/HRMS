...
public sealed class OvertimeService(IHrmsDbContext db, ITenantContext tenant, TimeProvider clock, IAttendanceAuthorizationService? authorization = null) : IOvertimeService
{
    ...
    public async Task<Result<OvertimeRequestDto>> CreateRequestAsync(OvertimeRequestRequest request, CancellationToken ct = default)
    {
        ...
        var committedCompOff = await db.CompOffEarnings.AsNoTracking().AnyAsync(x => x.TenantId == tid && x.EmployeeId == request.EmployeeId && x.SourceAttendanceDayId == day.Id && x.Status != CompOffEarningStatus.Rejected && x.Status != CompOffEarningStatus.Superseded, ct);
        if (committedCompOff) return Result<OvertimeRequestDto>.Conflict("DoubleBenefitDenied: the authoritative Attendance source is already committed to Comp-Off.");
