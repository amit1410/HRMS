...
public sealed class PayrollCalculationEngine(IHrmsDbContext db, ITenantContext tenant, TimeProvider clock, IStatutoryPayrollService? statutory = null, ILoanPayrollRecoveryResolver? loanRecovery = null, IReimbursementPayrollResolver? reimbursementResolver = null, IAttendancePayrollSnapshotResolver? attendance = null, IPayrollOvertimeSnapshotResolver? overtime = null) : IPayrollCalculationEngine
{
    ...
    private async Task<(PayrollResult? Result, PayrollCalculationError? Error)> CalculateEmployeeAsync(PayrollRun run, PayrollRunEmployee snapshot, Guid attemptId, int calculationVersion, CancellationToken ct)
    {
        ...
        PayrollOvertimeSnapshotContract? overtimeSnapshot = null;
        ...
        {
            var overtimeResult = await overtime.ResolveAsync(snapshot.EmployeeId, run.PayrollPeriod!.StartDate, run.PayrollPeriod.EndDate, ct);
            if (!overtimeResult.Succeeded) return (null, Error(run, snapshot, PayrollCalculationErrorCode.CalculationFailed, overtimeResult.Message));
            overtimeSnapshot = overtimeResult.Value;
        }
        ...
        if (overtimeSnapshot is { TotalApprovedMinutes: > 0, HourlyRateComponentId: Guid rateComponentId, MonthlyWorkMinutes: > 0 } && values.TryGetValue(rateComponentId, out var monthlyBasis))
        {
            var weightedMinutes = overtimeSnapshot.NormalDayMinutes * overtimeSnapshot.NormalDayMultiplier + overtimeSnapshot.WeekOffMinutes * overtimeSnapshot.WeekOffMultiplier + overtimeSnapshot.HolidayMinutes * overtimeSnapshot.HolidayMultiplier;
            var overtimeAmount = PayrollRoundingPolicy.RoundMoney(monthlyBasis / overtimeSnapshot.MonthlyWorkMinutes!.Value * weightedMinutes);
            if (overtimeAmount > 0) resultComponents.Add(new PayrollResultComponent { Id = Guid.NewGuid(), TenantId = run.TenantId, CalculationAttemptId = attemptId, ComponentCode = "OVERTIME", ComponentName = "Approved overtime", ComponentType = SalaryComponentType.Earning, CalculationType = SalaryStructureCalculationType.Manual, BaseAmount = monthlyBasis, Rate = weightedMinutes, UnproratedAmount = overtimeAmount, ProrationFactor = 1m, CalculatedAmount = overtimeAmount, IsEarning = true, IsTaxable = true, CalculationSequence = 880000, CalculationSource = "PayrollOvertimeSnapshot", CalculationMetadata = JsonSerializer.Serialize(new { overtimeSnapshot.SnapshotId, overtimeSnapshot.Version, overtimeSnapshot.AttendanceSnapshotId, overtimeSnapshot.AttendanceVersion }) });
        }
