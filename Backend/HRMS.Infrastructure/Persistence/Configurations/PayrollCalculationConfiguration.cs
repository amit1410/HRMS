...
public sealed class PayrollResultConfiguration : IEntityTypeConfiguration<PayrollResult>
{
    public void Configure(EntityTypeBuilder<PayrollResult> b)
    {
        ...
        b.Property(x => x.CurrencyCode).HasMaxLength(3).IsRequired(); b.Property(x => x.GrossEarnings).HasPrecision(18, 6); b.Property(x => x.TotalDeductions).HasPrecision(18, 6); b.Property(x => x.NetPay).HasPrecision(18, 6); b.Property(x => x.EmployerContributions).HasPrecision(18, 6); b.Property(x => x.ProrationFactor).HasPrecision(18, 8); b.Property(x => x.AttendanceEligibleDays).HasPrecision(18, 4); b.Property(x => x.AttendancePayableDays).HasPrecision(18, 4); b.Property(x => x.AttendanceLopDays).HasPrecision(18, 4); b.Property(x => x.Status).HasConversion<int>(); b.Property(x => x.ConcurrencyVersion).HasDefaultValue(1).IsConcurrencyToken();
...
public sealed class PayrollResultComponentConfiguration : IEntityTypeConfiguration<PayrollResultComponent>
{
    public void Configure(EntityTypeBuilder<PayrollResultComponent> b)
    {
        ...
        b.HasIndex(x => new { x.TenantId, x.PayrollResultId }); b.HasIndex(x => new { x.TenantId, x.SalaryComponentId }); b.HasIndex(x => new { x.TenantId, x.CalculationAttemptId }); b.HasIndex(x => new { x.TenantId, x.EmployeeLoanId, x.LoanInstallmentId }); b.HasOne(x => x.PayrollResult).WithMany(x => x.Components).HasForeignKey(new[] { "TenantId", "PayrollResultId" }).HasPrincipalKey(x => new { x.TenantId, x.Id }).OnDelete(DeleteBehavior.Cascade); b.HasOne(x => x.SalaryComponent).WithMany().HasForeignKey(new[] { "TenantId", "SalaryComponentId" }).HasPrincipalKey(x => new { x.TenantId, x.Id }).OnDelete(DeleteBehavior.Restrict); b.HasOne(x => x.EmployeeLoan).WithMany().HasForeignKey(new[] { "TenantId", "EmployeeLoanId" }).HasPrincipalKey(x => new { x.TenantId, x.Id }).OnDelete(DeleteBehavior.Restrict); b.HasOne(x => x.LoanInstallment).WithMany().HasForeignKey(new[] { "TenantId", "LoanInstallmentId" }).HasPrincipalKey(x => new { x.TenantId, x.Id }).OnDelete(DeleteBehavior.Restrict); b.HasOne(x => x.Tenant).WithMany().HasForeignKey(x => x.TenantId).OnDelete(DeleteBehavior.Restrict);
    }
}
