...
public sealed class AttendanceWorkflowController(IAttendanceWorkflowService service) : ControllerBase
{
    ...
    [HttpGet("manager/regularizations"), HasPermission(Permissions.Attendance.RegularizationApprove)] public async Task<ActionResult<ApiResponse<PagedResult<RegularizationDto>>>> ManagerRegularizations([FromQuery] AttendanceRegularizationQuery query, CancellationToken ct) => (await service.GetManagerRegularizationsAsync(query, ct)).ToActionResult();
