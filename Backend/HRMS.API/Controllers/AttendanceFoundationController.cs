...
public sealed class AttendanceFoundationController(IAttendanceFoundationService service) : ControllerBase
{
    ...
    [HttpGet("shift-applicability"), HasPermission(Permissions.Attendance.View)] public async Task<ActionResult<ApiResponse<PagedResult<ShiftApplicabilityDto>>>> Applicability([FromQuery] ShiftApplicabilityQuery query, CancellationToken ct) => (await service.GetApplicabilityAsync(query, ct)).ToActionResult();
    [HttpGet("shift-applicability/{id:guid}"), HasPermission(Permissions.Attendance.View)] public async Task<ActionResult<ApiResponse<ShiftApplicabilityDto>>> ApplicabilityById(Guid id, CancellationToken ct) => (await service.GetApplicabilityByIdAsync(id, ct)).ToActionResult();
    [HttpPut("shift-applicability/{id:guid}"), HasPermission(Permissions.Attendance.PatternManage)] public async Task<ActionResult<ApiResponse<ShiftApplicabilityDto>>> UpdateApplicability(Guid id, ShiftApplicabilityRequest request, CancellationToken ct) => (await service.UpdateApplicabilityAsync(id, request, ct)).ToActionResult();
    [HttpDelete("shift-applicability/{id:guid}"), HasPermission(Permissions.Attendance.PatternManage)] public async Task<ActionResult<ApiResponse<bool>>> DeleteApplicability(Guid id, CancellationToken ct) => (await service.DeleteApplicabilityAsync(id, ct)).ToActionResult();
    ...
    [HttpDelete("roster/{employeeId:guid}/{date}"), HasPermission(Permissions.Attendance.RosterManage)] public async Task<ActionResult<ApiResponse<bool>>> RemoveRoster(Guid employeeId, DateOnly date, CancellationToken ct) => (await service.RemoveRosterAsync(employeeId, date, ct)).ToActionResult();
    [HttpGet("roster"), HasPermission(Permissions.Attendance.View)] public async Task<ActionResult<ApiResponse<PagedResult<RosterGridRowDto>>>> Roster([FromQuery] RosterQuery query, CancellationToken ct) => (await service.GetRosterAsync(query, ct)).ToActionResult();
